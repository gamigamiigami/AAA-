...
(function (GG) {
  ...
  GG.reg({
    ...
    create: function (c) {
      ...
      /* 必要な回数から逆算する。
       *
       * 以前はレベル3で、押し切るのに 24 回ほど叩く必要があった。制限時間は
       * 3.4 秒なので毎秒 7 回。キーボードなら複数のキーを交互に叩けば届くが、
       * マウスのクリックでは届かない。同じ画面が、持っている道具で別の難度に
       * なっていた。それは難易度ではなく、遊べるかどうかの線引きになる。
       *
       * 一番遅い道具（クリック一本）で押し切れる回数を上限に置き、
       * レベル差はその中でつける。 */
      var drain = [0.14, 0.20, 0.26][c.diff - 1];
      var gain = [0.185, 0.155, 0.135][c.diff - 1];   // 8 / 11 / 14 回で押し切る
