...
見た目は**縁日**。提灯・暖簾・市松・青海波・色紙と判子で組んでいます。
...
- **アートディレクションは「縁日・和のポップ」**。高明度・低彩度の和色をフラットに置き、
  線は細い墨色。暗いグラデ背景・太い黒フチ・集中線・ツヤ玉といった西洋のゲーム語彙は使わない。
  背景の情報密度は放射線ではなく**地紋（青海波・市松・水玉）**で作る。
...
色は `GG.PAL`（和色パレット）、地面は `GG.A.GROUND` のプリセットから選ぶと全体の世界観が揃います。
